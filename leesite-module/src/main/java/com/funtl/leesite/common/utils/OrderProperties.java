...
public class OrderProperties extends Properties {
		...
		for (int x = 0; x < len; ) {
			...
			if (aChar == '\\') {
				...
				if (aChar == 'u') {
					...
					if (aChar == 't') outBuffer.append('\t'); /* ibm@7211 */

					else if (aChar == 'r') outBuffer.append('\r'); /* ibm@7211 */
					else if (aChar == 'n') {
						/*
						 ...
						 */
						outBuffer.append('\n'); /* ibm@8897 ibm@7211 */
					} else if (aChar == 'f') outBuffer.append('\f'); /* ibm@7211 */
					else
						/* ibm@7211 */
	...
	public synchronized void store(OutputStream out, String header) throws IOException {
		...
		@SuppressWarnings("rawtypes") List entrys = context.getCommentOrEntrys();
